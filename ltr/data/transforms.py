...
class TransformBase:
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params):
        ...
        coord_all = jt.var([[y1, y1, y2, y2], [x1, x2, x2, x1]])

        coord_transf = self.transform_coords(coord_all, image_shape, *rand_params).flip(0)
        tl = jt.min(coord_transf, dim=1)[0]
        sz = jt.max(coord_transf, dim=1)[0] - tl
...
class ToTensor(TransformBase):
    ...
    def transform_image(self, image):
        ...
        image = jt.var(image.transpose((2, 0, 1)))
...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        return jt.normalize(image, self.mean, self.std, self.inplace)
        # torch.Size([3, 256, 256])
...
class Blur(TransformBase):
    ...
    def __init__(self, sigma):
        ...
        x_coord = [jt.range(-sz, sz+1, dtype=jt.float32) for sz in self.filter_size]
    ...
    def transform_image(self, image):
        if jt.is_var(image):
            ...
            return jt.nn.conv2d(im1, self.filter[1], padding=(0,self.filter_size[1])).view(-1,sz[0],sz[1])
        else:
...
class RandomAffine(TransformBase):
    ...
    def transform_coords(self, coords, image_shape, do_flip, theta, shear_values, scale_factors):
        ...
        t_mat_tensor = jt.var(t_mat).float()

        coords_xy1 = jt.stack((coords[1, :], coords[0, :], jt.ones_like(coords[1, :])))

        coords_xy_t = jt.matmul(t_mat_tensor, coords_xy1)
