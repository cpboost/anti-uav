...
def transform_image_to_crop(box_in: jt.var, box_extract: jt.var, resize_factor: float,
                            crop_sz: jt.var) -> jt.var:
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    crop_sz = jt.var([output_sz, output_sz])
...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    if mask is not None:
        mask_crop_padded = jt.nn.pad(mask_crop, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant', value=0)
    ...
    if mask is not None:
        mask_out = \
        jt.nn.interpolate(mask_crop_padded[None, None], tuple(output_sz.flip(0).long().tolist()), mode='nearest')[0, 0]

    crop_box = jt.var([x1, y1, x2 - x1, y2 - y1])
...
def crop_and_resize(im, box, crop_bb, output_sz, mask=None):
    ...
    if mask is not None:
        mask_out = jt.nn.interpolate(mask_crop_padded[None, None], (output_sz[1], output_sz[0]), mode='nearest')[0, 0]
...
def transform_box_to_crop(box: jt.var, crop_box: jt.var, crop_sz: jt.var) -> jt.var:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
    args:
...
def iou(reference, proposals):
    ...
    # Intersection box
    tl = jt.max(reference[:, :2], proposals[:, :2])
    br = jt.min(reference[:, :2] + reference[:, 2:], proposals[:, :2] + proposals[:, 2:])
    sz = (br - tl).clamp(0)
...
def perturb_box(box, min_iou=0.5, sigma_factor=0.1):
    ...
    if not isinstance(c_sigma_factor, jt.var):
        c_sigma_factor = c_sigma_factor * jt.ones(4)

    perturb_factor = jt.sqrt(box[2] * box[3]) * c_sigma_factor
    ...
    for i_ in range(100):
        ...
        box_per = jt.var([c_x_per - 0.5 * w_per, c_y_per - 0.5 * h_per, w_per, h_per]).round()
...
def gauss_1d(sz, sigma, center, end_pad=0, density=False):
    k = jt.range(-(sz - 1) / 2, (sz + 1) / 2 + end_pad).reshape(1, -1)
    gauss = jt.exp(-1.0 / (2 * sigma ** 2) * (k - center.reshape(-1, 1)) ** 2)
    if density:
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    image_sz = jt.var(image_sz)
    feat_sz = jt.var(feat_sz)
    ...
    center = feat_sz * target_center_norm + 0.5 * \
             jt.var([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2])
    ...
    if density:
        sz = (feat_sz + jt.var(end_pad)).prod()
...
def gauss_density_centered(x, std):
    ...
    """
    return jt.exp(-0.5 * (x / std) ** 2) / (math.sqrt(2 * math.pi) * std)
...
def sample_gmm_centered(std, num_samples=1):
    ...
    k = jt.randint(num_components, (num_samples,), dtype=jt.int64)
    std_samp = std[0, :, k].t()

    # Sample
    x_centered = std_samp * jt.randn(num_samples, num_dims)
...
def sample_box_gmm(mean_box, proposal_sigma, gt_sigma=None, num_samples=1, add_mean_box=False):
    ...
    """
    center_std = jt.var([s[0] for s in proposal_sigma])
    sz_std = jt.var([s[1] for s in proposal_sigma])
    std = jt.stack([center_std, center_std, sz_std, sz_std])
    ...
    if gt_sigma is None or gt_sigma[0] == 0 and gt_sigma[1] == 0:
        gt_density = jt.zeros_like(proposal_density)
    else:
        std_gt = jt.var([gt_sigma[0], gt_sigma[0], gt_sigma[1], gt_sigma[1]]).view(1, 4)
        gt_density = gauss_density_centered(proposals_rel_centered, std_gt).prod(-1)

    if add_mean_box:
        proposals = jt.concat((mean_box, proposals))
        proposal_density = jt.concat((jt.var([-1]), proposal_density))
        gt_density = jt.concat((jt.var([1]), gt_density))
