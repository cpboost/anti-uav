...
class JittorTrackingSampler:
    ...
    def __getitem__(self, index):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = visible.type(jt.int64).sum().item() > 2 * (
...
class TransTSampler(JittorTrackingSampler):
    """ See TrackingSampler."""
...
class ModalSampler(JittorTrackingSampler):
    ...
    def __getitem__(self, index):
        ...
        data_RGB = TensorDict({'search_images': search_frames['RGB'],
                           'search_anno': search_anno['RGB']['bbox'],
                           'template_images': template_frames['RGB'],
                           'template_anno': template_anno['RGB']['bbox'],
                           'dataset': dataset.get_name(),
                           'template_class': meta_obj_template.get('object_class_name')})

        data_IR = TensorDict({'search_images': search_frames['IR'],
                           'search_anno': search_anno['IR']['bbox'],
                           'template_images': template_frames['IR'],
                           'template_anno': template_anno['IR']['bbox'],
                           'dataset': dataset.get_name(),
                           'template_class': meta_obj_template.get('object_class_name')})

        data['RGB'] = self.processing(data_RGB)
        data['IR'] = self.processing(data_IR)
        return data
