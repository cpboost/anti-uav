...
def rect_to_rel(bb, sz_norm=None):
    ...
    if sz_norm is None:
        ...
        c_rel = c / sz_norm
    sz_rel = jt.log(bb[...,2:])
    return jt.cat((c_rel, sz_rel), dim=-1)
...
def masks_to_bboxes(mask, fmt='c'):
    ...
    bboxes = jt.var(bboxes, dtype=jt.float32, device=mask.device)
...
def masks_to_bboxes_multi(mask, ids, fmt='c'):
    ...
    for id in ids:
        ...
        bb = jt.var(bb, dtype=jt.float32, device=mask.device)
        ...
        if fmt == 'v':
            ...
            bb = jt.concat((x1 + 0.5 * s, s), dim=-1)
        elif fmt == 't':
            bb = jt.concat((x1, s), dim=-1)
        else:
            raise ValueError("Undefined bounding box layout '%s'" % fmt)
        bboxes.append(bb)
