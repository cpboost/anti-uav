...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=6, embed_dim=96, norm_layer=None, pretrain_img_size=224,
...
def build_swin(name, frozen_backbone_layers=1):
    ...
    path = 'D:\study\\track\model\\swin_base_patch4_window12_384_22kto1k.pth'
    state_dict = jt.load(path, map_location='cuda')['model']
    _update_state_dict_(state_dict)
    backbone = SwinTransformer(out_indices=(1,2,3), frozen_stages=frozen_backbone_layers, **params)
...
class FPN_trans(nn.Module):
    ...
    def __init__(self,out_channels=256,out_size=16,dropout=0.1):
        ...
        self.att_min = nn.MultiheadAttention(embed_dim=256,num_heads=8,dropout=dropout)
        self.att_mid = nn.MultiheadAttention(embed_dim=256,num_heads=8,dropout=dropout)
        self.att_max = nn.MultiheadAttention(embed_dim=256,num_heads=8,dropout=dropout)
